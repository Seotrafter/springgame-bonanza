...
*/
    ...
    class Cart {
        constructor() {
            ...
            this.total = 0;
            this.count = 0;
            this.cartCounter = document.querySelector(".cart-header__count");
            this.cartItemsContainer = document.querySelector("[data-cart-items]");
            this.cartSummary = document.querySelector("[data-cart-summary]");
            this.cartEmpty = document.querySelector(".cart__empty");
            this.modal = document.querySelector(".cart-header__modal");
            this.modalItemsContainer = document.querySelector(".cart-header__items");
            this.modalEmptyMessage = document.querySelector(".cart-header__empty");
            this.modalTotalPrice = document.querySelector(".cart-header__total-price");
            this.purchaseModal = document.querySelector("[data-purchase-modal]");
            this.purchaseForm = document.querySelector("[data-purchase-form]");
        ...
        initEventListeners() {
            ...
            document.addEventListener("click", (e => {
                const clearCartBtn = e.target.closest("[data-clear-cart]");
                if (clearCartBtn) {
                    e.preventDefault();
                    this.clearCart();
                }
            }));
            const cartBtn = document.querySelector(".cart-header__button");
            if (cartBtn) cartBtn.addEventListener("click", (() => {
                this.showModal();
            }));
            const closeBtn = document.querySelector(".cart-header__close");
            ...
            document.addEventListener("click", (e => {
                ...
                const closeBtn = e.target.closest("[data-close-modal], .purchase-modal__close");
                if (closeBtn) {
                    e.preventDefault();
                    this.closePurchaseModal();
                    return;
                }
                if (this.purchaseModal && this.purchaseModal.classList.contains("active")) {
                    const modalContent = this.purchaseModal.querySelector(".purchase-modal__content");
                    if (modalContent && !e.target.closest(".purchase-modal__content")) this.closePurchaseModal();
                }
            }));
            this.handlePurchaseSubmit();
            document.addEventListener("click", (e => {
                const quantityBtn = e.target.closest("[data-quantity]");
                if (!quantityBtn) return;
                const productId = quantityBtn.closest("[data-product-id]").dataset.productId;
                const action = quantityBtn.dataset.quantity;
                if (action === "decrease") this.decreaseQuantity(productId); else if (action === "increase") this.increaseQuantity(productId);
            }));
            document.addEventListener("click", (event => {
                const removeBtn = event.target.closest(".cart-header__remove, .cart__item-remove, [data-remove]");
                if (!removeBtn) return;
                event.preventDefault();
                event.stopPropagation();
                const productContainer = removeBtn.closest("[data-product-id]");
        ...
        showPurchaseModal() {
            if (this.purchaseModal) {
                ...
                const overlayClickHandler = e => {
                    if (e.target.classList.contains("purchase-modal__overlay")) {
                        this.closePurchaseModal();
                        this.purchaseModal.removeEventListener("click", overlayClickHandler);
                    }
                };
                this.purchaseModal.addEventListener("click", overlayClickHandler);
            }
        }
        closePurchaseModal() {
            if (this.purchaseModal) {
        ...
        handlePurchaseSubmit() {
            ...
            this.purchaseForm.addEventListener("submit", (e => {
                ...
                document.querySelectorAll(".purchase-modal__error").forEach((error => {
                    error.textContent = "";
                }));
                [ "name", "email", "phone" ].forEach((field => {
                    if (!formData.get(field)) {
                        document.querySelector(`[data-error="${field}"]`).textContent = "Bu alan zorunludur";
                        isValid = false;
                    }
                }));
                if (isValid) {
        ...
        getProductData(productCard) {
            ...
            const price = priceElement ? parseInt(priceElement.dataset.productPrice) : 0;
            const imageElement = productCard.querySelector(".product-card__image-ibg img");
            let image = "";
            if (imageElement) image = imageElement.getAttribute("src").replace("@img/", "img/");
        ...
        addToCart(productCard) {
            ...
            if (existingItem) existingItem.quantity = (existingItem.quantity || 1) + 1; else this.items.push({
                ...productData,
                quantity: 1
            });
            this.updateCart();
        ...
        updateCart() {
            ...
            if (this.cartCounter) {
                this.cartCounter.textContent = this.count;
                this.cartCounter.classList.toggle("hidden", this.count === 0);
            }
            this.total = this.items.reduce(((sum, item) => sum + item.price * item.quantity), 0);
            if (this.modalItemsContainer && this.modalTotalPrice) if (this.items.length === 0) {
                this.modalItemsContainer.innerHTML = "";
                this.modalEmptyMessage.style.display = "block";
                this.modalTotalPrice.textContent = "₺0.00";
            } else {
                this.modalEmptyMessage.style.display = "none";
                this.modalItemsContainer.innerHTML = this.items.map((item => `\n          <div class="cart-header__item" data-product-id="${item.id}">\n            <div class="cart-header__item-image">\n              <img src="${item.image}" alt="${item.title}">\n            </div>\n            <div class="cart-header__item-info">\n              <h3 class="cart-header__item-title">${item.title}</h3>\n              <div class="cart-header__item-price">₺${item.price.toFixed(2)}</div>\n              <div class="cart-header__item-quantity">\n                <button class="cart-header__quantity-button" data-quantity="decrease">-</button>\n                <span class="cart-header__quantity-value">${item.quantity}</span>\n                <button class="cart-header__quantity-button" data-quantity="increase">+</button>\n              </div>\n            </div>\n            <button class="cart-header__remove" aria-label="Ürünü Kaldır">\n              <i class="fas fa-times"></i>\n            </button>\n          </div>\n        `)).join("");
                this.modalTotalPrice.textContent = `₺${this.total.toFixed(2)}`;
            }
            if (this.cartItemsContainer) if (this.items.length === 0) {
                ...
                this.cartItemsContainer.innerHTML = this.items.map((item => `\n          <div class="cart__item" data-cart-item data-product-id="${item.id}">\n            <div class="cart__item-image">\n              <img src="${item.image}" alt="${item.title}">\n            </div>\n            <div class="cart__item-info">\n              <h3 class="cart__item-title">${item.title}</h3>\n              <div class="cart__item-price">₺${item.price.toFixed(2)}</div>\n              <div class="cart__item-quantity">\n                <button class="cart__item-btn" data-quantity="decrease">-</button>\n                <span class="cart__item-count">${item.quantity}</span>\n                <button class="cart__item-btn" data-quantity="increase">+</button>\n              </div>\n            </div>\n            <button class="cart__item-remove" data-remove>\n              <i class="fas fa-times"></i>\n            </button>\n          </div>\n        `)).join("");
                if (this.cartSummary) {
                    const subtotal = this.total;
                    const tax = subtotal * .18;
                    const total = subtotal + tax;
                    this.cartSummary.innerHTML = `\n            <div class="cart__summary-header">\n              <h2>Sipariş Özeti</h2>\n            </div>\n            <div class="cart__summary-content">\n              <div class="cart__summary-row">\n                <span>Ara Toplam</span>\n                <span>₺${subtotal.toFixed(2)}</span>\n              </div>\n              <div class="cart__summary-row">\n                <span>KDV (18%)</span>\n                <span>₺${tax.toFixed(2)}</span>\n              </div>\n              <div class="cart__summary-total">\n                <span>Toplam</span>\n                <span>₺${total.toFixed(2)}</span>\n              </div>\n            </div>\n            <button class="button button--primary button--full" data-checkout>\n              <i class="fas fa-credit-card"></i>\n              Siparişi Tamamla\n            </button>\n          `;
                    const checkoutButton = this.cartSummary.querySelector("[data-checkout]");
        ...
        showAddToCartAnimation(productCard) {
            const productImage = productCard.querySelector(".product-card__image-ibg img");
            ...
            flyingImage.src = originalSrc;
            document.body.appendChild(flyingImage);
            const imageRect = productImage.getBoundingClientRect();
            const cartRect = cartIcon.getBoundingClientRect();
            gsapWithCSS.set(flyingImage, {
                position: "fixed",
                zIndex: 1e3,
                x: imageRect.left,
                y: imageRect.top,
        ...
        showEmptyState() {
            if (this.cartEmpty) this.cartEmpty.style.display = "block";
            if (this.modalEmptyMessage) this.modalEmptyMessage.style.display = "block";
            if (this.cartSummary) this.cartSummary.style.display = "none";
        }
    }
    ...
    function handleHelpfulClick(e) {
        ...
        if (matches && matches[1]) {
            ...
            if (button.classList.contains("active")) count++; else count--;
            countSpan.textContent = `Faydalı (${count})`;
        }
    }
    ...
    const initNewsletterForm = () => {
        ...
        form.addEventListener("submit", (async event => {
            ...
            document.querySelectorAll(".newsletter__form-error").forEach((error => {
                error.textContent = "";
            }));
            if (!name || name.trim().length < 2) {
                document.querySelector('[data-error="name"]').textContent = "Lütfen geçerli bir isim girin";
                isValid = false;
            }
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (!email || !emailRegex.test(email)) {
                document.querySelector('[data-error="email"]').textContent = "Lütfen geçerli bir e-posta adresi girin";
                isValid = false;
            }
            if (!preferences || preferences.trim().length < 3) {
                document.querySelector('[data-error="preferences"]').textContent = "Lütfen tercihlerinizi belirtin";
                isValid = false;
            }
            if (!consent) {
                document.querySelector('[data-error="consent"]').textContent = "Devam etmek için onaylamanız gerekiyor";
                isValid = false;
            }
            if (isValid) {
